...
pub mod trading_pair_azero {
    ...
    pub struct TradingPairAzero {
        ...
        // LP PSP22 share tokens address
        lp_psp22_address: AccountId,
        // LP PSP22 share tokens name
        lp_psp22_name: Option<String>
    ...
    impl TradingPairAzero {
        ...
        pub fn new(
            ...
            let salt = version.to_le_bytes(); // Salt to provide to init new contract
            // Init new PSP22 smart contract to act as LP share tokens
            let lp_psp22 = MyPsp22LpRef::new(
                lp_token_name.clone(),
                Option::Some(String::from("LPST").into()),
                12u8,
                Self::env().account_id())
                        .endowment(0)
                        .code_hash(psp22_hash)
                        .salt_bytes(salt)
                        .try_instantiate();

            let lp_psp22_address = lp_psp22.unwrap().expect("Failed Instantiation").get_account_id(); // Address of the newly created LP PSP22 token
            let lp_psp22_name = lp_token_name.clone(); // Name of the LP PSP22 tokens
       ...
       pub fn provide_to_pool(
        ...
        // cross contract call to LP PSP22 share tokens contract to transfer LP PSP22 tokens to the caller
        if PSP22Ref::transfer(&self.lp_psp22_address, caller, new_caller_shares, vec![]).is_err()
        {
       ...
       pub fn withdraw_specific_amount(
            ...
            // caller current LP PSP22 share tokens balance
            let caller_current_lp_psp22_balance: Balance = PSP22Ref::balance_of(&self.lp_psp22_address, caller);

            // validating that the caller has more than the given number of shares (PSP22).
            if caller_current_lp_psp22_balance < shares {
                return Err(TradingPairErrors::CallerInsufficientLPBalance)
            }

            // TPA LP PSP22 allowance to take from the caller to withdraw 
            let lp_psp22_allowance: Balance =
                PSP22Ref::allowance(&self.lp_psp22_address, caller, Self::env().account_id());

            // making sure that the trading pair contract has enough allowance to withdraw.
            if lp_psp22_allowance < shares {
                return Err(TradingPairErrors::NotEnoughAllowance)
            }
            ...
            // cross contract call to LP PSP22 contract to transfer LP PSP22 tokens to the TPA contract
            if PSP22Ref::transfer_from_builder(
                &self.lp_psp22_address,
                caller,
                Self::env().account_id(),
                shares,
                vec![],
       ...
       #[ink(message)]
       pub fn get_psp22_redeemable_amount(
           &mut self,
       )   -> Result<Balance, TradingPairErrors> {
        ...
        pub fn redeem_redeemable_amount(
            ...
            let current_account_overall_psp22_staking_rewards = self.account_overall_staking_rewards
                .get(&caller)
                .unwrap_or(0);
        ...
        pub fn get_account_overall_staking_rewards(
           ...
           let psp22_overall_amount = self.account_overall_staking_rewards.get(&owner).unwrap_or(0);
        ...
        /// function to get LP psp22 share tokens contract address (self)
        #[ink(message)]
        pub fn get_tpa_lp_psp22_address(&self) -> AccountId {
            self.lp_psp22_address
        }

        /// function to get LP psp22 name (self)
        #[ink(message)]
        pub fn get_tpa_lp_psp22_name(&self) -> String {
            self.lp_psp22_name.clone().unwrap()
        }
